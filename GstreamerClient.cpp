...
void createGstreamerClient (std::string monitorSource, std::string host, int aport, int vport) {
  ...
  //pulsesrc name=psrc
  pipeline = gst_parse_launch ("pulsesrc name=psrc ! vorbisenc ! rtpvorbispay ! udpsink name=asink \
		  ximagesrc ! ffmpegcolorspace ! ffenc_mpeg4 ! rtpmp4vpay ! udpsink name=vsink", &error);
  if (!pipeline) {
  ...
  g_object_set (asink, "host", host.c_str(), NULL);
  g_object_set (asink, "port", aport, NULL);
  g_object_set (acaps, "caps", acaps, NULL);

  vsink = gst_bin_get_by_name (GST_BIN (pipeline), "vsink");
  g_object_set (vsink, "host", host.c_str(), NULL);
  g_object_set (vsink, "port", vport, NULL);
  g_object_set (vcaps, "caps", vcaps, NULL);
